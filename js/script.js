...
function initContactPage() {
  ...
  const transmissionSteps = document.getElementById('transmission-steps');
  ...
  form.addEventListener('submit', e => {
    ...
    if (!phone) { showMiniToast('ERROR: PHONE FIELD REQUIRED'); return; }

    // Show transmission overlay with step-by-step animation
    const steps = [
      { text: 'INITIALIZING SECURE CHANNEL...', delay: 400 },
      { text: 'ENCRYPTING PAYLOAD WITH AES-256...', delay: 600 },
      { text: 'ROUTING THROUGH HALZIZ PROXY NETWORK...', delay: 700 },
      { text: 'ESTABLISHING CONNECTION TO <span class="highlight">contact@halziz.com</span>...', delay: 800 },
      { text: '<span class="success">TRANSMISSION COMPLETE</span>', delay: 500 }
    ];

    if (transmissionOverlay && transmissionSteps) {
      transmissionSteps.innerHTML = '';
      ...
      steps.forEach((step, i) => {
        ...
        setTimeout(() => {
          const el = document.createElement('div');
          el.className = 'transmission-step';
          el.innerHTML = `> ${step.text}`;
          transmissionSteps.appendChild(el);
          requestAnimationFrame(() => el.classList.add('active'));
          if (i === steps.length - 1) {
            setTimeout(() => {
              transmissionOverlay.classList.remove('active');
              // Open mailto and show notification
              const mailtoSubject = encodeURIComponent(`Contact Form: ${subject}`);
              const mailtoBody = encodeURIComponent(
                `Name: ${name}\nEmail: ${email}${phone ? `\nPhone: ${phone}` : ''}\nSubject: ${subject}\n\nMessage:\n${message}`
              );
              const mailtoLink = document.createElement('a');
              mailtoLink.href = `mailto:contact@halziz.com?subject=${mailtoSubject}&body=${mailtoBody}`;
